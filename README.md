...
git -C aee-conformance checkout 55ee73321cd40edd2b4a814948506a60074543a2
cargo run --locked --release -- aee-conformance/vectors --json fresh.json
python3 scripts/compare-report.py fresh.json reports/suite-revision-2.json
```

The checkout is pinned deliberately. `main` moves, and a later revision would run
a different corpus against the 138/138 claim on this page, which is the one thing
a reproduction recipe must not do quietly.
